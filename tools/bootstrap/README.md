...
The starter itself also changes: a file it shipped last time can be gone or renamed now. Every re-bootstrap lists such orphans, each marked `unmodified` or `modified locally` (it differs from what the tool last wrote, which a formatter or a dependency bot causes as readily as a hand edit). Nothing is deleted unless you pass `--prune`; add `--review` to choose per file. Your code directory is never examined. A repo bootstrapped before this inventory existed has nothing recorded, so its first re-bootstrap lists, as possibly left over, any file in a tool directory that the starter does not ship; one of those may be a file of yours, so `--prune` deletes them only together with `--review`. To keep an orphan and stop the report, delete its line from the `[orphans]` table in `.omniglot_bootstrap.toml`.
