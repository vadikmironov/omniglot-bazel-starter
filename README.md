...
|---|---|---|
| Rust | criterion + pprof-rs (`PProfProfiler(Output::Protobuf)`) | gperftools `tcmalloc` heap profiler, driven over FFI by `modules/rust_workloads/mem/prof_dump.rs` — the same mechanism C++ uses. Linking it is the whole integration: tcmalloc interposes `malloc`/`free`, so Rust's allocations are captured without a `#[global_allocator]`. It records every allocation rather than sampling, so all four buckets are exact and directly comparable with C++. Its predecessor, `jemalloc_pprof`, could report only live bytes — jemalloc removed `opt.prof_accum` in 5.0.0, putting cumulative counts permanently out of reach. Because the capture rides the C++ shard, Rust memory profiling requires the `cpp` language; Rust CPU profiling does not. |
