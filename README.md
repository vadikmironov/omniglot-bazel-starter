...
- **Cross-language profiling** — CPU + memory flamegraphs from tagged bench targets through a hermetic capture → pprof → [inferno](https://github.com/jonhoo/inferno) pipeline, with a Gazelle extension generating the workload targets and an opt-in host-`perf` sampler
...
To profile your own code, add `# gazelle:profiling` to the package's BUILD file and run `bazel run //:profile_gen`: workload sources under `benches/` (CPU) and `mem/` (memory) get their tagged targets generated, wired to the package's library, and reaped when sources disappear. Hand-written targets carrying the tags work equally; the generator never touches packages without the directive.

Capture is per-language; everything downstream (folded stacks, rendering, the runner) is shared:

| Language | CPU bench (`benches/`) | Memory workload (`mem/`) |
|---|---|---|
| Rust | [criterion](https://github.com/criterion-rs/criterion.rs) benches sampled by [pprof-rs](https://github.com/tikv/pprof-rs), hooked in through criterion's `Profiler` trait by `//tools/profile/criterion_pprof` — a dep on every generated bench target, installed with `Criterion::default().with_profiler(PProfProfiler::new(100))` and writing `profile.pb` into criterion's per-benchmark directory. pprof-rs ships an equivalent behind its own `criterion` feature; keeping the hookup here means that feature's criterion 0.5 dependency doesn't pin your benches. | gperftools `tcmalloc` heap profiler, driven over FFI by `modules/rust_workloads/mem/prof_dump.rs` — the same mechanism C++ uses. Linking it is the whole integration: tcmalloc interposes `malloc`/`free`, so Rust's allocations are captured without a `#[global_allocator]`. It records every allocation rather than sampling, so all four buckets are exact and directly comparable with C++. Its predecessor, `jemalloc_pprof`, could report only live bytes — jemalloc removed `opt.prof_accum` in 5.0.0, putting cumulative counts permanently out of reach. Because the capture rides the C++ shard, Rust memory profiling requires the `cpp` language; Rust CPU profiling does not. |
| Go | `testing.B` benches in `benches/bench_*_test.go` (stdlib capture via `-test.cpuprofile`) | `runtime/pprof` heap profile dumped to `$MEMPROF_OUT` after `runtime.GC()` (shim: `modules/go_workloads/mem/prof_dump.go`). Cross-platform; records the GC-managed live heap by sampled allocation site, and uniquely carries *both* the cumulative and live views in one artifact. Sampled every 32 KiB (`runtime.MemProfileRate`, matching Rust's `lg_prof_sample`); the runtime scales values back up, so each run prints its effective sample count and error bar. A workload must hold its result across the dump (`runtime.KeepAlive`) or the dump's own GC collects it. |
